...
Flutter (sejak versi 3.7+) memiliki fitur bawaan untuk membaca file `.env` secara langsung saat kompilasi (*compile-time*) menggunakan flag `--dart-define-from-file` **tanpa perlu menduplikasi file atau menginstal package pihak ketiga**.

#### 1. Membaca Variabel dalam Kode Dart (`mobile/lib/core/config/env.dart`):
```dart
class AppEnv {
  // Nilai dibaca saat build-time dari file .env
...
#### 2. Menjalankan / Build Flutter via CLI:
```bash
...
#### 3. Konfigurasi VS Code Debugger (`.vscode/launch.json`):
Tambahkan argumen `toolArgs` agar setiap kali menekan tombol `F5` (Run), Flutter otomatis memuat `.env` root:
```json
{
  "version": "0.2.0",
  "configurations": [
