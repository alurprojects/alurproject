...
- [ ] Setup *environment* FastAPI dan deploy ke **Vercel Serverless (Tahap 1)**. *Catatan:* Perhatikan limit eksekusi 10 detik; persiapkan fallback arsitektur manual (tanpa LangGraph) ke Cloudflare Workers jika limit ini menjadi kendala kritis pada endpoint real-time.
...
- [ ] Rangkai agent menggunakan **LangGraph** dengan *conditional routing*. Gunakan **Groq sebagai primary LLM** untuk jalur real-time agar inference berjalan super cepat (< 10 detik).
...
- [ ] Buat tabel `ai_insights`, `task_suggestions`, dan tabel pendukung cron `cron_batch_state`.
- [ ] Implementasikan helper function SQL `users_active_this_week()` di Supabase.
- [ ] Implementasikan **Reflection Agent** yang membaca data gabungan (`tasks` + `conversation_logs`). Gunakan **Gemini 2.0 Flash sebagai primary LLM** di jalur cron ini untuk reasoning yang lebih baik.
...
- [ ] Implementasikan mekanisme **Batching** pada `weekly-reflection` (memproses user bertahap) dan gunakan parameter `?only_active=true` untuk menghindari *timeout* Vercel saat user bertambah.
- [ ] Setup `pg_cron` di Supabase untuk eksekusi panggilan HTTP POST ke endpoint internal.
